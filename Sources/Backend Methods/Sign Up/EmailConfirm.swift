...
extension Backend {
    public func emailConfirm(email: String, token: String, deviceToken: String? = nil, appSecurityTokenId: String? = nil, callback: (Result<ConfirmAuthResponse, BackendError<String>>) -> Void) async {
        ...
        let request: Result<ConfirmAuthResponse, NetworkError> = await Request.post(url: "\(config.baseUrl)/\(config.language)/api/v2/user/email/confirm/\(token)", body: EmailAuthRequest(email: email, deviceToken: deviceToken, appSecurityTokenId: appSecurityTokenId))
